...
static const SystemEntry systems[] = {
    ...
    { "arcade", "Arcade / MAME", "mame", NULL, "zip" },
...
static void load_rom_list(const SystemEntry *sys){free_rom_list();free_all_rom_list();char path[512];snprintf(path,sizeof(path),"./roms/%s/",sys->dir_name);DIR*dir=opendir(path);if(!dir)return;int cap=20;all_rom_list=calloc(cap,sizeof(RomEntry));struct dirent*e;
    while((e=readdir(dir))){if(!strcmp(e->d_name,".")||!strcmp(e->d_name,".."))continue;char fp[1024];snprintf(fp,sizeof(fp),"./roms/%s/%s",sys->dir_name,e->d_name);struct stat st;if(stat(fp,&st))continue;
        int accept=0;
        if(!strcmp(sys->mame_sys,"mame")){
            accept=S_ISDIR(st.st_mode)||(S_ISREG(st.st_mode)&&has_allowed_extension(e->d_name,sys->allowed_exts));
        }else{
            accept=S_ISREG(st.st_mode)&&has_allowed_extension(e->d_name,sys->allowed_exts);
        }
        if(accept){if(all_rom_count>=cap){cap*=2;all_rom_list=realloc(all_rom_list,cap*sizeof(RomEntry));}all_rom_list[all_rom_count].display_name=strdup(e->d_name);all_rom_list[all_rom_count++].rom_path=strdup(fp);}
    }
    if(strcmp(sys->mame_sys,"mame")!=0){rewinddir(dir);while((e=readdir(dir))){if(!strcmp(e->d_name,".")||!strcmp(e->d_name,".."))continue;char sp[1024];snprintf(sp,sizeof(sp),"./roms/%s/%s",sys->dir_name,e->d_name);struct stat st;if(stat(sp,&st)||!S_ISDIR(st.st_mode))continue;DIR*sd=opendir(sp);if(!sd)continue;struct dirent*se;while((se=readdir(sd)))if(se->d_type==DT_REG&&has_allowed_extension(se->d_name,sys->allowed_exts)){if(all_rom_count>=cap){cap*=2;all_rom_list=realloc(all_rom_list,cap*sizeof(RomEntry));}char fp[1024];snprintf(fp,sizeof(fp),"%s/%s",sp,se->d_name);all_rom_list[all_rom_count].display_name=strdup(se->d_name);all_rom_list[all_rom_count++].rom_path=strdup(fp);}closedir(sd);}}
    closedir(dir);filter_rom_list();}
...
static void handle_joystick_input(const SDL_Event*e){
    ...
    if(e->type==SDL_EVENT_JOYSTICK_AXIS_MOTION&&e->jaxis.axis==1){
        int v=e->jaxis.value;
        if(v>-AXIS_DEADZONE&&v<AXIS_DEADZONE){
            joystick_axis_latched=0;
            return;
        }
        if(!joystick_axis_latched&&now>=last_input_time+INPUT_COOLDOWN_MS){
            move_selection(v<0?-1:1);
            joystick_axis_latched=1;
            last_input_time=now;
        }
        return;
    }

    if(e->type==SDL_EVENT_JOYSTICK_BUTTON_DOWN&&e->jbutton.button==0&&now>=last_input_time+INPUT_COOLDOWN_MS){
        activate_selection();
        last_input_time=now;
    }
}
